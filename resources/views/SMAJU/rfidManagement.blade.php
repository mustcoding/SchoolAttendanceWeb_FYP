...
<body>
  ...
  <main id="main" class="main">
    ...
    <div class="alert alert-success alert-dismissible fade show" role="alert1" style="display: none;">
      Here is the RFID you were searching for.
      <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    </div>

    <div class="alert alert-danger alert-dismissible fade show" role="alert2" style="display: none;">
      Houston... we can't find the RFID you search.
      <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    </div>

    <div class="card">
        <div class="card-body">
            <h5 class="card-title">Search RFID</h5>
    
            <form class="row g-3 align-items-center">
              <div class="col-md-6">
                  <div class="input-group">
                      <input type="text" class="form-control me-2" id="services" placeholder="Enter RFID Number">
  ...
  <script>
    ...
    function fetchData() {
      fetch('http://127.0.0.1:8000/rfid/getAll')
        ...
        const row = `<tr>
                      ...
                      <td>
                        <div class="button-column">
                          <button type="button" class="btn btn-primary" onclick="editRFID(${item.id})">Edit</button>
                          <button type="button" class="btn btn-danger" onclick="deleteRFID(${item.id})">Delete </button>
                        </div>
                      </td>
                    </tr>`;
    ...
    rows.forEach(function (row) {
        ...
        var rfid_number = row.querySelector('td:nth-child(2)').textContent.toLowerCase();
        var student_name = row.querySelector('td:nth-child(4)').textContent.toLowerCase();

        // Convert both the company name and the search term to lowercase for comparison
        if (rfid_number.includes(searchTerm) || student_name.includes(searchTerm)) {
  ...
  function editRFID(rfid_id){
     // Get the tourismServiceId from the URL
    
     window.open(`editRFID?id=${rfid_id}`,'_self');
  }

  function deleteRFID(rfid_id)
  {
    var is_Delete=1;
    ...
    fetch('http://127.0.0.1:8000/rfid/delete/'+rfid_id, {
      ...
      .then(data => {
            ...
            window.location.href = "http://127.0.0.1:8000/rfidManagement";
