                ...
                <div class="tab-pane fade pt-3" id="profile-change-password">
                  ...
                  <form onsubmit ="return confirmChangePassword(this);">
                    ...
                    <div class="row mb-3">
                      ...
                      <div class="col-md-8 col-lg-9">
                        <input name="newpassword" type="password" class="form-control" id="newPassword1">
                      </div>
                    </div>
                    ...
                    <div class="row mb-3">
                      ...
                      <div class="col-md-8 col-lg-9">
                        <input name="renewpassword" type="password" class="form-control" id="confirmPassword">
                      </div>
                    </div>
...
<script>
    function confirmEdit(form) 
    {
        ...
        .then(response => {
          if (!response.ok) {
            hrow new Error(`HTTP error! Status: ${response.status}`);
          }
...
<script>
  function confirmChangePassword(form) {
    ...
    if (newPassword1 === confirmPassword) {
      ...
      // You might want to add code here to inform the user or take appropriate action
    }
    ...
    if(passwordSame){
      ...
      fetch('SchoolAttendance_php/changePassword.php', 
      {
            method: 'POST', // Use the POST method
            headers: {
            'Content-Type': 'application/json' // Set the content type to JSON
            },
            body: JSON.stringify(data1) // Convert the data object to a JSON string
      })
        .then(response => response.json())
        .then(data => {
           ...
           else
           {
                ...
                .then(response => response.json())
                  .then(data => {
                      ...
                      console.log("ID SEND : ", data.staffId);

                      if(passwordChanged){
                        document.querySelector('.alert.alert-success.alert-dismissible.fade.show[role="alert1"]').style.display = 'block';
                        document.querySelector('.alert.alert-danger.alert-dismissible.fade.show[role="alert2"]').style.display = 'none';
                        document.querySelector('.alert.alert-danger.alert-dismissible.fade.show[role="alert3"]').style.display = 'none';
                        // Delay redirection to previous page for 5 seconds
                        setTimeout(function () {
                          location.reload();
                        }, 7000);
                      }
    ...
    // Add your logic here to update the user profile or perform other actions

    // Return false to prevent the form from submitting (since you're handling it with JavaScript)
    return false;
  }
</script>
