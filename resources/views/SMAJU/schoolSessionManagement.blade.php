...
<body>
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1>School Session Management</h1>
    </div><!-- End Page Title -->

    <div class="alert alert-success alert-dismissible fade show" role="alert1" style="display: none;">
      Here is the School Session you were searching for.
      <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    </div>

    <div class="alert alert-danger alert-dismissible fade show" role="alert2" style="display: none;">
      Houston... we can't find the School Session you search.
      <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    </div>

    <div class="card">
        <div class="card-body">
            <h5 class="card-title">Search School Session</h5>
    
            <form class="row g-3 align-items-center">
              <div class="col-md-6">
                  <div class="input-group">
                      <input type="text" class="form-control me-2" id="services" placeholder="Find School Session">
  ...
  <script>
        ...
        const row = `<tr>
                      ...
                      <td>
                        <div class="button-column">
                          <button type="button" class="btn btn-primary" onclick="editSchoolSession(${item.id})">Edit</button>
                          <button type="button" class="btn btn-danger" onclick="deleteSchoolSession(${item.id})">Delete </button>
                        </div>
                      </td>
                    </tr>`;
    ...
    rows.forEach(function (row) {
        ...
        var staffName = row.querySelector('td:nth-child(2)').textContent.toLowerCase();
        var position = row.querySelector('td:nth-child(4)').textContent.toLowerCase();

        // Convert both the company name and the search term to lowercase for comparison
        if (staffName.includes(searchTerm) || position.includes(searchTerm)) {
  ...
  function editSchoolSession(ss_id){
     // Get the tourismServiceId from the URL
    
     window.open(`editSchoolSession?id=${ss_id}`,'_self');
  }

  function deleteSchoolSession(ss_id)
  {
    var is_Delete=1;
    ...
    fetch('http://127.0.0.1:8000/SchoolSession/delete/'+ss_id, {
      ...
      .then(data => {
            ...
            window.location.href = "http://127.0.0.1:8000/schoolSessionManagement";
