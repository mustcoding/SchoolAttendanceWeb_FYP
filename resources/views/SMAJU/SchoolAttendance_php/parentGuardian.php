...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        if (isset($jsonbody->parentGuardianName) &&
            ...
            $stmt = $db->prepare("SELECT parentGuardianId FROM parentguardian WHERE parentGuardianName=:parentGuardianName 
			AND nickname=:nickname AND phoneNumber=:phoneNumber");
            $stmt->execute([
                ':parentGuardianName' => $jsonbody->parentGuardianName,  
            ...
            if ($stmt->rowCount() > 0) {
               ...
               $response->parentGuardianId = $result['parentGuardianId'];
               $response->error = "Parent already registered";

            } else {
                // Insert the new user
                $stmt = $db->prepare("INSERT INTO parentguardian (`parentGuardianName`,`nickname`,`username`,`password`,`phoneNumber`,`address`) 
                VALUES (:parentGuardianName, :nickname, :username, :password, :phoneNumber, :address)");
				...
				$response->parentGuardianId = $lastInsertId;
