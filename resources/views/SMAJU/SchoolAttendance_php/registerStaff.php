...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        if (isset($jsonbody->staffName) &&
            ...
            $stmt = $db->prepare("SELECT * FROM staff WHERE staffName=:staffName 
			AND username=:username");
            $stmt->execute([':staffName' => $jsonbody->staffName,  
            ...
            if ($stmt->rowCount() > 0) {
               ...
               // Bad Request
                $response->error = "Staff already registered";
            } else {
                // Insert the new user
                $stmt = $db->prepare("INSERT INTO staff (`staffName`,`nickname`,`username`,`password`,`position`) VALUES (:staffName, :nickname, :username, :password, :position)");

				$stmt->execute([
					':staffName' => $jsonbody->staffName,
