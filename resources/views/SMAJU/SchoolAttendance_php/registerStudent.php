...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        if (isset($jsonbody->studentName) &&
			isset($jsonbody->dateOfBirth) &&
			isset($jsonbody->parentGuardianId) &&
			isset($jsonbody->rfidId)) {
			
            // Check if the email already exists
            $stmt = $db->prepare("SELECT * FROM student WHERE studentName=:studentName 
			AND dateOfBirth=:dateOfBirth AND parentGuardianId=:parentGuardianId");
            $stmt->execute([
                ':studentName' => $jsonbody->studentName,  
			    ':dateOfBirth'=>$jsonbody->dateOfBirth,
                'parentGuardianId'=>$jsonbody->parentGuardianId
            ]);
			
            if ($stmt->rowCount() > 0) {
                http_response_code(400);
               // Bad Request
                $response->error = "Student already registered";
            } else {
                // Insert the new user
                $stmt = $db->prepare("INSERT INTO student (`studentName`,`dateOfBirth`,`parentGuardianId`,`rfidId`) 
                VALUES (:studentName, :dateOfBirth, :parentGuardianId, :rfidId)");

				$stmt->execute([
					':studentName' => $jsonbody->studentName,
					':dateOfBirth' => $jsonbody->dateOfBirth,
					':parentGuardianId' => $jsonbody->parentGuardianId,
					':rfidId' => $jsonbody->rfidId
				]);
				...
				$response->studentId = $lastInsertId;
