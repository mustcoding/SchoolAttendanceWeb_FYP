...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        if ($jsonbody && isset($jsonbody->rfidNumber) && isset($jsonbody->type)) 
            {

            $rfidNumber = $jsonbody->rfidNumber;
            $type = $jsonbody->type;  

            // Perform database insertion
            $stmt = $db->prepare("INSERT INTO rfid (rfidNumber, type) 
                                   VALUES  (:rfidNumber, :type)");
            ...
            $response->success = "RFID added successfully";
        } else {
