...
<li class="nav-item">
          ...
          <div class="card">
              <div class="card-body">
                ...
                <form id="schoolSessionForm" class="row g-3 needs-validation" validate>
                    ...
                    <div class="col-12">
                        <label for="form" class="form-label">Teacher Name</label>
                        ...
                        <div class="invalid-feedback">
                            Please select Classroom Teacher!
                          </div>
                    </div>
                   
                    <div class="text-center">
                        <button type="button" class="btn btn-primary" onclick="saveSchoolSession(value)">Next</button>
  ...
  <script>
    ...
    function saveSchoolSession()
    {
        ...
        var availableSize = values[1];

        if (schoolSessionId === '' || classroomId === '' || staffId=== '')
        {
        ...
        fetch('http://127.0.0.1:8000/classroom/get-classroom/'+classroomId, 
        {
            ...
            .then(data => {
              ...
              console.log("JEEIEI: ",data);
                      
              var SchoolSessionClass = {
                schoolSessionId: schoolSessionId,
                classroomId: classroomId,
                staffId: staffId,
                max_capacity: data.classroom.max_capacity,
                available_size: availableSize,
                form_number:data.classroom.form_number
              };

              sessionStorage.setItem('SchoolSessionClass', JSON.stringify(SchoolSessionClass));

              window.location.href = 'http://127.0.0.1:8000/add-student-in-classroom';
...
<script>
    ...
    data.forEach((item) => {
        ...
        option.textContent = "Start Date: "+ item.start_date + " : End Date: "+item.end_date+ ": Year: "+item.year;
  ...
  <script>
      function fetchTeacher(){
          fetch('http://127.0.0.1:8000/staff/all-data')
              .then(response => response.json())
              .then(data => {
  ...
  <script>
      ...
      const teacher = document.getElementById('staffId');
      const currentValue = teacher.value; // Store the current selected value
      teacher.innerHTML='';
      ...
      emptyOption.setAttribute('disabled', true);
      emptyOption.setAttribute('hidden', true);
      teacher.appendChild(emptyOption);
